...
const InternshipsPage = () => {
  ...
  useEffect(() => {
    const fetchInternships = async () => {
      try {
        setLoading(true);
        const response = await fetch('https://remotive.com/api/remote-jobs?category=internship');
    ...
    <div className="min-h-screen flex flex-col bg-[var(--xai-bg-primary)] text-[var(--xai-text-primary)] transition-colors duration-300">
      ...
      <main className="flex-1 p-4 md:p-8 max-w-[1400px] mx-auto w-full">
        <div className="mb-8 bg-[var(--xai-bg-secondary)]/60 backdrop-blur-sm p-6 rounded-2xl border border-[var(--xai-border)] transition-all duration-300">
          <h2 className="text-3xl mb-2 text-[var(--xai-text-primary)] font-bold">Remote Internship Opportunities</h2>
          <p className="text-[var(--xai-text-secondary)] mb-4">Discover remote internships from companies around the world</p>
          
          <div className="mt-6">
            {loading ? (
              <div className="flex justify-center items-center h-64">
                <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-[var(--xai-text-primary)]"></div>
                <span className="ml-3 text-[var(--xai-text-primary)]">Loading internships...</span>
              </div>
            ) : error ? (
              ...
              <div className="text-center py-12">
                <p className="text-[var(--xai-text-secondary)] text-lg">No internships found at the moment.</p>
                ...
                {internships.map((internship) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-4">
                      <h3 className="text-xl font-bold text-[var(--xai-text-primary)] mb-2">{internship.title || 'Untitled Position'}</h3>
                      {internship.logo_url && (
                        <img 
